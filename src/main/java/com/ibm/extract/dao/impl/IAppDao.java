...
public interface IAppDao {
	...
	//CDMS
	@Select("select trim(serial)||trim(cc) from bp.ww_emp where trim(serial)||trim(cc) in( select distinct trim(serial)||trim(cc) from req.access_req where status='GRANTED' and request_id in(  select request_id from PROV.REQ_BLUEGROUPS_TRANSACTIONS where app_code='CDMS' and bluegroup_name in ('External Reporting', 'Global Financing', 'Policy', 'SEC Tech Support') and status='COMPLETE'))")
	...
	@Select("select trim(serial)||trim(cc) user,trim(mgrserial)||trim(mgrcc) manager from bp.ww_emp where trim(serial)||trim(cc) in (${serialCcs})")
